...
try:
    ...
    from .Predictor import Predictor
class FE_NeuralODE_Residuals(Predictor):
    def __init__(self, state_size:int, action_size:int, use_actions:bool=True, hidden_size=51, n_basis=100):
        ...
        self.model = torch.nn.Sequential( # this models learns the average dynamics function
            torch.nn.Linear(input_size, hidden_size),
            torch.nn.ReLU(),
            torch.nn.Linear(hidden_size, hidden_size),
            torch.nn.ReLU(),
    ...
    def predict(self, states:tensor, actions:tensor, example_states:tensor, example_actions:tensor, example_next_states:tensor, average_function_only=False) -> Tuple[tensor, dict]:
        ...
        if not average_function_only:
            # compute average function w/o gradients
            with torch.no_grad():
                average_function_example_dif = self.rk4(self.model, example_states, example_actions, absolute=False)
                average_function_dif = self.rk4(self.model, states, actions, absolute=False)
            ...
            return torch.einsum("fks,feks->fes", encodings, individual_encodings) + states + average_function_dif, {}
        else:
            return self.rk4(self.model, states, actions, absolute=True), {}
...
if __name__ == "__main__":
    ...
    model = FE_NeuralODE_Residuals(5, 3, False)
    states, actions, example_states, example_actions, example_next_states = torch.rand(7, 10, 5), torch.rand(7, 10, 3), torch.rand(7, 10, 5), torch.rand(7, 10, 3), torch.rand(7, 10, 5)
    print(torch.sum(torch.tensor([p.numel() for p in model.parameters()])).item(), "parameters")
    out1 = model.predict(states, actions, example_states, example_actions, example_next_states)
    torch.save(model.state_dict(), "/tmp/model.pt")
