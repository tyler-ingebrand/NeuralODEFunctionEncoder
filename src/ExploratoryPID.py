...
# taken from safe control gym
# Look for the TODO tag for the things that are changed

class ExploratoryPID(BaseController):
    '''PID Class.'''
    ...
    def select_action(self, obs, info=None):
        ...
        if self.env.QUAD_TYPE == 2:
            ...
            if self.env.TASK == Task.TRAJ_TRACKING:
                ...
                target_pos = np.array([obs[0], obs[2], obs[4]]) # TODO This target location + noise is different from normal
                target_pos += np.random.normal(0, 0.1, 3)
                target_vel = np.array([0, 0, 0])
